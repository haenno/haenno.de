...
if [ $(git -C $HAENNO_SCRIPT_DIR rev-list HEAD...origin/main --count) != 0 ]; then
    ...
    sudo docker-compose --project-directory $HAENNO_SCRIPT_DIR stop
    sudo docker-compose --project-directory $HAENNO_SCRIPT_DIR build
    sudo docker-compose --project-directory $HAENNO_SCRIPT_DIR up -d
